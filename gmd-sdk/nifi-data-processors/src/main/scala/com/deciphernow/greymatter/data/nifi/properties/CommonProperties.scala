...
trait CommonProperties extends PropertyUtils with ErrorHandling {
  ...
  protected def dynamicProperty(name: String) = buildPropertyWithValidators(List(StandardValidators.ATTRIBUTE_EXPRESSION_LANGUAGE_VALIDATOR), name, "", ExpressionLanguageScope.FLOWFILE_ATTRIBUTES).dynamic(true).build()
  ...
  protected def parseDynamicProperties(implicit context: ProcessContext, flowFile: Option[FlowFile] = None) = context.getProperties.asScala.toMap.filter(_._1.isDynamic).keys.map { propertyDescriptor =>
