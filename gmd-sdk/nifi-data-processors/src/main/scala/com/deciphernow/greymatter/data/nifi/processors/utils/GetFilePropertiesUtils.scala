...
trait GetFilePropertiesUtils extends GmDataClient[IO] with GetFilePropertiesProperties with ProcessorRelationships with ProcessorUtils {
  ...
  def getFileProps(implicit context: ProcessContext, session: ProcessSession, flowFile: FlowFile, logger: ComponentLog, clientRef: Ref[IO, Client[IO]], cs: ContextShift[IO]) = {
    for {
      ...
      propertiesEither <- userFolderEither.flatTraverse(userFolder => getPropsAndStatus(s"$userFolder/$filePath/$fileName", headers, rootUrl, client).attempt)
